...
def segments():
    ...
    insight = f"生命周期阶段中，'{max_segment['TenureGroup']}'的流失率最高，达到 {max_segment['流失率']:.1%}，建议重点关注该阶段用户的流失原因。"
    
    # 转换为列表，方便模板渲染
    segments = segment_df.to_dict('records')
    
    return render_template('segments.html', segments=segments, insight=insight)
